...
// Refresh MCP configs from AWS (new endpoint)
app.post('/api/mcp/refresh', async (req, res) => {
  try {
    logger.info('Manual refresh requested');
    
    const result = await mcpManager.refreshConfigs();
    
    res.json({
      success: true,
      message: 'MCP configs refreshed from AWS',
      ...result
    });
...
app.get('/api/mcp/servers', (req, res) => {
  try {
    ...
    res.json({
      ...
      registered: mcpManager.getRegisteredServers().length
    });
...
app.listen(port, '0.0.0.0', () => {
  ...
  // Initialize MCP servers from AWS
  setTimeout(async () => {
    try {
      await mcpManager.initializeServers();
      logger.info('✅ MCP servers initialized from AWS');
    } catch (error) {
      logger.error('❌ Failed to initialize MCP servers:', error);
    }
  }, 5000); // Wait 5s for server to start

  // Periodic sync from AWS (every 5 minutes)
  setInterval(async () => {
    try {
      logger.info('🔄 Periodic sync from AWS...');
      await mcpManager.syncFromAWS();
    } catch (error) {
